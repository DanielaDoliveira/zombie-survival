...
namespace com.Daniela.Player
{
    ...
    public class PlayerLife : MonoBehaviour, IDead, IHealthy
    {
        ...
        public void AddLife(int life_amount)
        {
            _status.Life += life_amount;

            if (_status.Life >= _status.StartLife)
            {
                Debug.LogWarning("Cannot add more life!");
                _status.Life = _status.StartLife;
            }
            else
            {
